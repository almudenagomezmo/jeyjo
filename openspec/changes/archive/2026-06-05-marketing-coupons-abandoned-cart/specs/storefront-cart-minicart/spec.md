...
The storefront SHALL provide a `/cart` route listing all cart lines with quantity controls, subtotal, shipping preview, discount coupon input validated via `POST /api/cart/coupon` (replacing demo validation), and primary CTAs that navigate to `/checkout` when the cart has lines. The checkout flow implementation lives in `storefront-checkout-shipping`; this capability only links to it.
