...
The home SHALL render a grid of featured categories with name, visual glyph, and link to `/c/{slug}`, using CMS-configured featured categories when present.

#### Scenario: Featured categories from global home

- **WHEN** the CMS home global lists three published category relationships
- **THEN** the home displays exactly those three categories in configured order

#### Scenario: Fallback to navigation roots when unset

- **WHEN** the CMS home global has no featured categories configured
- **THEN** the home displays up to six root categories from the storefront navigation tree
