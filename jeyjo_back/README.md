...
1. `cd docker && docker compose up -d` to arrancar PostgreSQL local
1. `cd .. && pnpm dev` to start the dev server
...
El proyecto incluye un contenedor Docker con PostgreSQL 16. Para usarlo:
...
Esto creará un contenedor `jeyjo-postgres` con:
...
Los datos son persistentes gracias a un volumen Docker (`postgres_data`).
...
El archivo `.env` ya está configurado para usar la base de datos local. La variable `DATABASE_URL` apunta al contenedor Docker de PostgreSQL.

## Supabase

Cuando tengas las credenciales de Supabase, solo necesitas editar el `.env` y reemplazar estos valores:
...
# Storage (S3)
SUPABASE_ENDPOINT=https://TU_PROJECT.supabase.co/storage/v1/s3
...
El plugin de S3 (`@payloadcms/storage-s3`) está configurado en `src/plugins/index.ts`. Mientras las variables de Supabase tengan valores placeholder (`xxxx`, `tu_`), el plugin **no se activa** y los archivos se guardan localmente en `public/media/`.

En cuanto reemplaces los placeholders con credenciales reales de Supabase, el plugin se activa automáticamente y los uploads se redirigen al bucket S3 de Supabase. No necesitas tocar ningún otro archivo.

### Detener Docker al migrar a Supabase

Cuando ya estés usando Supabase, puedes detener el contenedor local:
...
2. Framework automático: **Next.js** (lo detecta solo)
3. Añade las siguientes **Environment Variables** en Vercel:

| Variable | Valor |
|---|---|
| `DATABASE_URL` | `postgresql://postgres:TU_PASS@db.TU_PROJECT.supabase.co:5432/postgres` |
| `PAYLOAD_SECRET` | Un string aleatorio seguro |
| `NEXT_PUBLIC_SERVER_URL` | `https://tu-dominio.vercel.app` |
| `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` | Tu clave pública de Stripe |
| `STRIPE_SECRET_KEY` | Tu clave secreta de Stripe |
| `STRIPE_WEBHOOKS_SIGNING_SECRET` | Tu webhook secret de Stripe |
| `SUPABASE_BUCKET` | `ecommerce-media` |
| `SUPABASE_REGION` | `us-east-1` |
| `SUPABASE_ENDPOINT` | `https://TU_PROJECT.supabase.co/storage/v1/s3` |
| `SUPABASE_ACCESS_KEY_ID` | Tu Access Key de Supabase S3 |
| `SUPABASE_SECRET_ACCESS_KEY` | Tu Secret Key de Supabase S3 |

> Las variables `NEXT_PUBLIC_*` deben ir con el prefijo para que Next.js las exponga al cliente.

### Base de datos en producción

Puedes usar cualquiera de estas opciones:

- **Supabase Postgres** (recomendado si ya usas Supabase): Usa `DATABASE_URL` con la connection string de Supabase (puerto `5432` o `6543` con pooler)
- **Vercel Postgres**: Instala `@payloadcms/db-vercel-postgres` y cambia la variable a `POSTGRES_URL`

### Cambios incluidos en el código

El proyecto ya tiene todo lo necesario configurado:

- ✅ **`sharp`** — activado en `payload.config.ts` para procesamiento de imágenes
- ✅ **`serverURL`** — se auto-configura con `NEXT_PUBLIC_SERVER_URL` o la URL de Vercel
- ✅ **`cors` / `csrf`** — permiten peticiones desde el dominio de producción y local
- ✅ **`@payloadcms/storage-s3`** — listo para usar con Supabase Storage
- ✅ **`getServerSideURL`** — ya maneja `VERCEL_PROJECT_PRODUCTION_URL` automáticamente

### Notas importantes

- **Migraciones**: Antes de hacer deploy, genera las migraciones localmente con `pnpm payload migrate:create` y súbelas al repo. En producción ejecuta `pnpm payload migrate`.
- **Variables de entorno en local**: Sigue usando el `.env` con Docker para desarrollo. Vercel usará sus propias variables.
- **Stripe webhooks**: En producción, apunta los webhooks de Stripe a `https://tu-dominio.vercel.app/api/payments/stripe/webhooks`.
...
This project includes a Docker setup for PostgreSQL. See [Configuración Local con Docker](#configuración-local-con-docker) for instructions.
