...
The home route SHALL render a minimal placeholder (no ERP or Payload catalog dependency) proving the shell works.
...
The storefront SHALL include layout components `TopBar`, `Header`, `Footer`, and `Container` ported from jeyjo-next structure, rendered from the root layout.
