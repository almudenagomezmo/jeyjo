...
`apps/cms/.env.example` SHALL list variables required for audit console and MFA behavior (e.g. `SUPABASE_SERVICE_ROLE_KEY`, optional `MFA_GRACE_DAYS` for local dev only), and dashboard KPI configuration (`TZ`, `DASHBOARD_LOW_STOCK_THRESHOLD`, `TOP_SALES_WINDOW_DAYS`).
...
- **AND** dashboard-related variables are listed with documented defaults
